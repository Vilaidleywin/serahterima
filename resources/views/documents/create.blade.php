    ...
    <form method="POST" action="{{ route('documents.store') }}">
      ...
      <div class="row g-3">
        <div class="col-md-4">
          ...
          <input type="text" name="number" class="form-control search" value="{{ old('number') }}" placeholder="Misal: ST-005">
          @error('number') <div class="text-danger small">{{ $message }}</div> @enderror
        </div>

        <div class="col-md-8">
          <label class="form-label fw-semibold">Judul Dokumen</label>
          <input type="text" name="title" class="form-control search" value="{{ old('title') }}" placeholder="Judul dokumen">
          @error('title') <div class="text-danger small">{{ $message }}</div> @enderror
        </div>

        <div class="col-md-6">
          <label class="form-label fw-semibold">Penerima</label>
          <input type="text" name="receiver" class="form-control search" value="{{ old('receiver') }}" placeholder="Nama penerima">
          @error('receiver') <div class="text-danger small">{{ $message }}</div> @enderror
        </div>

        <div class="col-md-6">
          <label class="form-label fw-semibold">Nominal (Rp)</label>
          <input type="number" name="amount" class="form-control search" value="{{ old('amount') }}" placeholder="cth: 2000000">
